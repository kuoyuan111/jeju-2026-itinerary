...
- **實作方式**: 全部 inline 在 `index.html` 中（CSS + HTML + JavaScript）
- **功能清單**:
  - ✅ 導入 Leaflet.js 地圖庫，並使用高質感的 CartoDB.Positron 無 API 限制之底圖。
  ...
  - ✅ 列印最佳化：地圖支援 `@media print` 樣式，在輸出列印/匯出 PDF 時會自動隱藏，維持版面乾淨。
...
**最後更新**: 2026/06/16  
**版本**: v1.6 - 導入 Leaflet 互動地圖與雙向滾動聯動機制  
**狀態**: 🟡 第一階段核心功能開發中（已完成地圖視覺化與聯動，進行圖片上傳規劃中）
